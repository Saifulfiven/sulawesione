        ...
        <div class="col-12">
          <div class="card mb-4">
            ...
            <div class="card-header pb-0 position-relative mb-5">
              <h6>Data Pemilihan Walikota / Bupati</h6>
                ...
                <table class="table align-items-center mb-0" id="tabel-data">
                  <thead>
                    <tr>
                      ...
                      <th class="text-secondary">Nama Kandidat</th>
                    ...
                    <tbody>
                      <?php $no = 1; ?>
                      @foreach ($Dapilkab as $dapil)  
                        <tr>
                          <td> {{ $no }}</td>
                          <td>{{ $dapil->namakandidat }}</td>
                          <td>{{ $dapil->namakabupaten }}</td>
                          <td>{{ $dapil->created_at }}</td>
                          <td class="text-center">
                            <a href="{{ url('admin/dapil/ubah/'.$dapil->id) }}" class="btn btn-sm btn-primary">Edit</a>
                            <a href="{{ url('admin/dapil/hapus/'.$dapil->id) }}"
              ...
              <div class="table-responsive p-3">
                <table class="table align-items-center mb-0" id="tabel-data-pilgub">
                  <thead>
                    ...
                    <tbody>
                      <?php $no = 1; ?>
                      @foreach ($Dapilprov as $dapil)  
                        <tr>
                          <td> {{ $no }}</td>
                          <td>{{ $dapil->namakandidat }}</td>
                          <td>{{ $dapil->namaprovinsi }}</td>
                          <td>{{ $dapil->created_at }}</td>
                          <td class="text-center">
                            <a href="{{ url('admin/dapil/ubah/'.$dapil->id) }}" class="btn btn-sm btn-primary">Edit</a>
                            <a href="{{ url('admin/dapil/hapus/'.$dapil->id) }}"
